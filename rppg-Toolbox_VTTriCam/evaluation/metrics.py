...
def calculate_metrics(predictions, labels, config):
    ...
    save_path_dir  = os.path.join(config.LOG.PATH, config.TEST.DATA.EXP_DATA_NAME)
    if not os.path.exists(save_path_dir):
        os.makedirs(save_path_dir, exist_ok=True)

    print (f"save_path_dir is --- {save_path_dir}\n")
    ...
    print (f"Subj_name-gt_hr-pred_hr\n")
    for index in tqdm(predictions.keys(), ncols=80):
        prediction = _reform_data_from_dict(predictions[index])
        label = _reform_data_from_dict(labels[index])
        
        print("done loading the files")
        ...
        for i in range(0, len(prediction), window_frame_size):
            ...
            if config.INFERENCE.EVALUATION_METHOD == "peak detection":
                gt_hr_peak, pred_hr_peak, SNR, macc, s_labels, s_preds = calculate_metric_per_video(
                ...
                gt_hr_fft, pred_hr_fft, SNR, macc, s_labels, s_preds = calculate_metric_per_video(
                ...
                gt_hr_all.append(gt_hr_fft)
                pred_hr_all.append(pred_hr_fft)
                names_all.append(str(index))
                
                # print (f"Subj_name-gt_hr-pred_hr\n")
                print(f"PARTICIPANT_{str(index)}-{gt_hr_fft}-{pred_hr_fft}")
                

            else:
                raise ValueError("Inference evaluation method name wrong!")
            
            np.save((save_path_dir+"/PARTICIPANT_"+str(index)+f"_true.npy" ),s_labels)
            np.save((save_path_dir+"/PARTICIPANT_"+str(index)+f"_pred.npy" ),s_preds)
            
    
    print(f"\n ----------Printing All HR values--------------")
    print (f"Subj_name-gt_hr-pred_hr\n")
    for subj_name, gt_hr_val, pred_hr_val,  in zip(names_all, gt_hr_all,pred_hr_all):
        print(f"PARTICIPANT_{subj_name}-{gt_hr_val}-{pred_hr_val}")
    ...
    if config.INFERENCE.EVALUATION_METHOD == "FFT":
        ...
        for metric in config.TEST.METRICS:
            if metric == "MAE":
                ...
                print("FFT MAE (FFT Label): {:.3f} +/- {:.3f}".format(MAE_FFT, standard_error))
            elif metric == "RMSE":
                ...
                print("FFT RMSE (FFT Label): {:.3f} +/- {:.3f}".format(RMSE_FFT, standard_error))
            elif metric == "MAPE":
                MAPE_FFT = np.mean(np.abs((predict_hr_fft_all - gt_hr_fft_all) / gt_hr_fft_all)) * 100
                standard_error = np.std(np.abs((predict_hr_fft_all - gt_hr_fft_all) / gt_hr_fft_all)) / np.sqrt(num_test_samples) * 100
                print("FFT MAPE (FFT Label): {:.3f} +/- {:.3f}".format(MAPE_FFT, standard_error))
            elif metric == "Pearson":
                
                np.save((save_path_dir+"/predict_hr_fft_all.npy"), predict_hr_fft_all)
                np.save((save_path_dir+"/gt_hr_fft_all.npy"), gt_hr_fft_all)
                
                Pearson_FFT = np.corrcoef(predict_hr_fft_all, gt_hr_fft_all)
                correlation_coefficient = Pearson_FFT[0][1]
                standard_error = np.sqrt((1 - correlation_coefficient**2) / (num_test_samples - 2))
                print("FFT Pearson (FFT Label): {:.3f} +/- {:.3f}".format(correlation_coefficient, standard_error))
            elif metric == "SNR":
                SNR_FFT = np.mean(SNR_all)
                standard_error = np.std(SNR_all) / np.sqrt(num_test_samples)
                print("FFT SNR (FFT Label): {:.3f} +/- {:.3f} (dB)".format(SNR_FFT, standard_error))
            elif metric == "MACC":
                MACC_avg = np.mean(MACC_all)
                standard_error = np.std(MACC_all) / np.sqrt(num_test_samples)
                print("FFT MACC (FFT Label): {:.3f} +/- {:.3f}".format(MACC_avg, standard_error))
            elif "AU" in metric:
                pass
            elif "BA" in metric:  
        ...
        for metric in config.TEST.METRICS:
            if metric == "MAE":
                ...
                print("Peak MAE (Peak Label): {:.3f} +/- {:.3f}".format(MAE_PEAK, standard_error))
            elif metric == "RMSE":
                # Calculate the squared errors, then RMSE, in order to allow
                # for a more robust and intuitive standard error that won't
                # be influenced by abnormal distributions of errors.
                ...
                standard_error = np.sqrt(np.std(squared_errors) / np.sqrt(num_test_samples))
                print("PEAK RMSE (Peak Label): {:.3f} +/- {:.3f}".format(RMSE_PEAK, standard_error))
            elif metric == "MAPE":
                ...
                print("PEAK MAPE (Peak Label): {:.3f} +/- {:.3f}".format(MAPE_PEAK, standard_error))
            elif metric == "Pearson":
                Pearson_PEAK = np.corrcoef(predict_hr_peak_all, gt_hr_peak_all)
                correlation_coefficient = Pearson_PEAK[0][1]
                standard_error = np.sqrt((1 - correlation_coefficient**2) / (num_test_samples - 2))
                print("PEAK Pearson (Peak Label): {:.3f} +/- {:.3f}".format(correlation_coefficient, standard_error))
            elif metric == "SNR":
                SNR_PEAK = np.mean(SNR_all)
                standard_error = np.std(SNR_all) / np.sqrt(num_test_samples)
                print("PEAK SNR (PEAK Label): {:.3f} +/- {:.3f} (dB)".format(SNR_PEAK, standard_error))
            elif metric == "MACC":
                MACC_avg = np.mean(MACC_all)
                standard_error = np.std(MACC_all) / np.sqrt(num_test_samples)
                print("PEAK MACC (PEAK Label): {:.3f} +/- {:.3f}".format(MACC_avg, standard_error))
            elif "AU" in metric:
                pass
            elif "BA" in metric:
