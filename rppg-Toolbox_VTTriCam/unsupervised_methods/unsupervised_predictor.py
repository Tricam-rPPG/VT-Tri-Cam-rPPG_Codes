...
def unsupervised_predict(config, data_loader, method_name):
    ...
    SNR_all = []
    MACC_all = []
    exp_name_method_name = f"{config.UNSUPERVISED.DATA.EXP_DATA_NAME}_{method_name}"
    save_path_dir  = os.path.join(config.LOG.PATH, exp_name_method_name)
        ...
        for idx in range(batch_size):
            ...
            for i in range(0, len(BVP), window_frame_size):
                ...
                if config.INFERENCE.EVALUATION_METHOD == "peak detection":
                    gt_hr, pre_hr, SNR, macc, s_labels, s_preds = calculate_metric_per_video(BVP_window, label_window, diff_flag=False,
                    ...
                    names_all.append(subj_id)
                elif config.INFERENCE.EVALUATION_METHOD == "FFT":
                    gt_fft_hr, pre_fft_hr, SNR, macc, s_labels, s_preds = calculate_metric_per_video(BVP_window, label_window, diff_flag=False,
                    ...
                    gt_hr_all.append(gt_fft_hr)
                    pred_hr_all.append(pre_fft_hr)
                    names_all.append(subj_id)
                else:
                    raise ValueError("Inference evaluation method name wrong!")
            # PARTICIPANT_915_S2_T2_pred.npy
            gt_sav_name = f"{save_path_dir}/PARTICIPANT_{subj_id}_true.npy" 
            pred_sav_name = f"{save_path_dir}/PARTICIPANT_{subj_id}_pred.npy" 
            np.save(gt_sav_name,s_labels)
            np.save(pred_sav_name,s_preds)
            
            # print("done saving files")
            
            print(f"PARTICIPANT_{names_all[-1]}-{gt_hr_all[-1]}-{pred_hr_all[-1]}")
    ...
    if config.INFERENCE.EVALUATION_METHOD == "peak detection":
        ...
        for metric in config.UNSUPERVISED.METRICS:
            if metric == "MAE":
                ...
                standard_error = np.std(SNR_all) / np.sqrt(num_test_samples)
                print("FFT SNR (FFT Label): {:.3f} +/- {:.3f} (dB)".format(SNR_PEAK, standard_error))
            elif metric == "MACC":
                MACC_avg = np.mean(MACC_all)
                standard_error = np.std(MACC_all) / np.sqrt(num_test_samples)
                print("MACC (avg): {:.3f} +/- {:.3f}".format(MACC_avg, standard_error))
            elif "BA" in metric:
